...
void calc(){
    ...
    long long kr1 = lower_bound(xai.begin(), xai.end(), make_pair(sx, -1)) - xai.begin();
    long long kr2 = lower_bound(yai.begin(), yai.end(), make_pair(sy, -1)) - yai.begin();
    long long vn = max(kr1-1, 0);
...
/*    for(auto x : mas){
    ...
    while(!q.empty()){
        ...
        for(auto x : prk) {
            ...
            long long i2 = max(i1-1, 0); i1 = min(i1 + 1, (long long)xai.size()-1);
            long long d1, d2;
            ...
            long long i4 = max(i3-1, 0); i3 = min(i3 + 1, (long long)yai.size()-1);
            long long d3, d4;
