...
long long getInv(long long x, long long md = mod1){
    return pw(x, md-2);
}
void precalc(bool mas[][dydis], long long hsh[][dydis], int n, int m){
    for(int i = 0; i < n; i++){
        long long pref = 0;
        for(int j = 0; j < m; j++){
            pref = (pref + mas[i][j] * getInd(i, j) % mod1) % mod1;
...
long long getVal(int e, int s, int sz, int n, int m, long long hsh[][dydis]){
    ...
    long long vl = ((da + kv) % mod1 - (ka + dv) % mod1 + mod1) % mod1;
    vl = vl * inv[e][s] % mod1;
    return vl;
